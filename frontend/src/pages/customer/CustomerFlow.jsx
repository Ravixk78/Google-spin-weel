...
const CustomerFlow = () => {
  ...
  // Google Auth Email Input state
  const [googleEmail, setGoogleEmail] = useState(customerUser?.email || 'google.customer@majlisaloud.ae');
  const [googleName, setGoogleName] = useState(customerUser?.name || '');

  // Step 5: Google Login Handler
  const handleGoogleLogin = async (e) => {
    if (e) e.preventDefault();
    const cleanEmail = (googleEmail || '').trim() || 'google.customer@majlisaloud.ae';
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {currentStep === 1 && (
        <div className="glass-panel border-gold-400/20 rounded-2xl p-6 md:p-8 max-w-md mx-auto text-center shadow-lg">
          ...
          <form onSubmit={handleGoogleLogin} className="space-y-4 text-left">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">
                {lang === 'ar' ? 'بريد Google الإلكتروني:' : 'Google Email Address:'}
              </label>
              <input
                type="email"
                required
                ...
                className="w-full py-2.5 px-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-xs focus:outline-none focus:border-gold-400 placeholder-slate-500 font-mono"
              />
            </div>

            <button
              type="submit"
              className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-800 font-semibold text-sm transition-all shadow-md mt-2"
            >
